...
# -*- coding: utf-8 -*-
#
# subs_gb_learn.py — Subtractor-only Goldbach finder (learned band ordering)
#
...
# - Early break when p >= n-2 (then q<=2 forever)
...
#     * We divide the subtractor prime array into contiguous bands of size B.
#     * For a given digit size D, we sort bands by learned success-rate (CTR).
#     * We then run the *normal sequential loop* per band (no per-item sampling).
#     * After each n, we update band stats and write back to disk.
#
...
import argparse, math, random, csv, json, os
...
def search_subtractor_learned(
    ...
    *,
    ...
    """
    Subtractors with learned band order. We iterate bands in 'band_order';
    inside each band we go sequentially (ascending p). Returns:
      (found, checks_used, hit_band_index or None)
    'checks' counts subtractor primes that pass the wheel filter and receive q checks.
    """
    if n % 2 or n < 4:
        return False, 0, None
    ...
    for b in band_order:
        ...
        if start >= end:
            continue
        # scan this band sequentially
        for idx in range(start, end):
            p = primes[idx]
            # early break: once p >= n-2, remaining q <= 2
            if p >= n - 2:
                return False, checks, None
            # wheel prefilter (cheap, using residue cache)
            wheel_ok = True
...
def rationale() -> str:
    return (
        "\n*** Subtractor-only Goldbach (learned bands) ***\n\n"
        "Given even n, we try ascending subtractor primes p so that q=n-p is prime.\n"
        "Speedups:\n"
        "  • Wheel residue prefilter: skip p forcing q divisible by small primes.\n"
        "  • Pre-sieve q by many small primes before BPSW.\n"
        "  • Early break when p >= n-2 (remaining q<=2).\n"
        "  • Precomputed residues p % r (per r) to avoid repeated mod ops.\n"
        "  • Learned band ordering: the subtractor list is partitioned into bands.\n"
        "    For each digit size D, we keep per-band trials/successes in a cache.\n"
        "    For a new n with D digits, we sort bands by success-rate and scan bands\n"
        "    in that order — zero hot-loop overhead.\n"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        prog="subs-gb-learn",
        description="Subtractor-only Goldbach with wheel/presieve/early-break and learned band ordering."
    )
    ...
    def get_stats_for_digits(D: int) -> Dict[str, List[int]]:
        ...
        # If cache had fewer bands (e.g., different ceiling/band-size), resize safely
        for k in ("trials","successes"):
            arr = cache[key][k]
    ...
    def order_bands_for_digits(D: int) -> Tuple[List[int], Dict[str, List[int]]]:
        stats = get_stats_for_digits(D)
        # Optional decay per use to keep cache fresh
        apply_decay(stats, max(0.0, min(1.0, args.decay)))
        sc = band_scores(stats["trials"], stats["successes"], smoothing=max(0.0, args.smoothing))
        # produce band order by descending score, tie-break by band index
    ...
    def record_band_updates(stats: Dict[str, List[int]], visited_band_trials: Dict[int, int], hit_band: Optional[int]) -> None:
        # Increment trials by how many q-checks we actually did in each visited band
        # Increment successes by 1 for the hit band (once)
    ...
    def run_for_digits(D: int, trials: int) -> Tuple[int,int,List[int]]:
        ...
        # fetch ordering & stats for this digit size
        band_order, stats = order_bands_for_digits(D)

        for _ in range(trials):
            n = random_even_with_digits(D, rng)
            # We’ll track how many q-checks we perform per band for updating CTR stats
            visited_band_trials: Dict[int, int] = {}

            # Wrap the band-ordered search to also count trials per band
            checks_total = 0
            found = False
            hit_band = None

            if n % 2 == 0 and n >= 4:
                # Iterate bands in the learned order
                for b in band_order:
                    start = b * band_size
                    end   = min(start + band_size, total_items)
                    if start >= end:
                        continue
                    band_checks = 0
                    # sequential within band
                    for idx in range(start, end):
                        p = subtractor_primes[idx]
                        if p >= n - 2:
                            # update visited stats and exit early
                            if band_checks:
                                visited_band_trials[b] = visited_band_trials.get(b, 0) + band_checks
                            checks_total += band_checks
                            found = False
                            hit_band = None
                            # update cache for this n
                            record_band_updates(stats, visited_band_trials, hit_band)
                            break
                        # wheel prefilter
                        wheel_ok = True
                        for r, nr in build_wheel_residues_for_n(n, wheel_list).items():
                            if res_cache[r][idx] == nr:
                                wheel_ok = False; break
                        if not wheel_ok:
                            continue
                    ...
                    if found or checks_total >= args.subs_max_checks or (start < end and subtractor_primes[start] >= n - 2):
                        break
    ...
    D = args.digits
    print(f"# Subtractor-only (LEARNED) | D={D} | count={args.count} | seed={args.seed}")
    print(f"# subs_ceiling={args.subs_ceiling} | subs_max_checks={args.subs_max_checks} "
